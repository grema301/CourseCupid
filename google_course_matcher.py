...
try:
    ...
    print(json.dumps({"error": f"Failed to configure Gemini API: {e}. Ensure GEMINI_API_KEY environment variable is set."}),
          file=sys.stderr)
    sys.exit(1)
...
def embed_courses(courses):
    ...
    for i in range(0, len(texts), batch_size):
        ...
        try:
            # Gemini Embeddings API to get vectors for a batch of courses
            ...
            print(f"Error embedding courses with Gemini API: {e}",
                  file=sys.stderr)
            return None
...
def get_top_matches(user_input, courses, course_embeddings):
    ...
    try:
        ...
        print(f"Error embedding user input with Gemini API: {e}",
              file=sys.stderr)
        return []
